...
public class Calculator {
    ...
    public static void main(String[] args) {
        ...
        String vvod2 = vvod.replaceAll(" ", "");
        System.out.print("Результат выражения: " + vvod2);

        String operStr = " ";
        char[] symbol = new char[10];
        char oper = '+';
        for (int i=1; i < vvod2.length(); i++) {
            symbol[i] = vvod2.charAt(i);
            if ( symbol[i] == '+'){
                oper = '+'; operStr = "\\+";
            }
            if ( symbol[i] == '-'){
        ...
        int result = 0;
        int resultArab = 0;
        String[] numbers = vvod2.split(operStr);
        if (numbers.length > 2) {
        ...
        try{
            ...
            System.out.println("=? \nСтрока не является математической операцией! Попробуйте снова.");
            System.exit(0);
        }
        if (num1 == 0 | num2 == 0) {
            result = 0;
            try {
                ...
                System.out.println("=? \nИспользуются одновременно разные системы счисления! Попробуйте снова.");
            ...
            try {
                ...
                if (result == 0) {
                    System.out.println("=? \nВ римской системе нет отрицательных чисел! Попробуйте снова.");
                    System.exit(0);
                }
